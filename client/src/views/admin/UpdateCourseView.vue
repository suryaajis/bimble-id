  ...
  <div class="p-8">
    <div class="max-w-2xl">
      <div class="flex items-center gap-3 mb-6">
        <RouterLink :to="`/admin/courses/${$route.params.courseId}`" class="text-gray-400 hover:text-gray-600">
      ...
      <div v-else class="card p-6">
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <RouterLink :to="`/admin/courses/${$route.params.courseId}`" class="btn-outline flex-1 text-center">Cancel</RouterLink>
          </div>
        </form>
      </div>
    </div>
...
const form = ref({ name: '', description: '', price: 0, thumbnailUrl: '', difficulty: '', status: 'active', CategoryId: '' })
...
async function handleSubmit() {
  ...
  error.value = ''
  try {
    await api.put(`/admin/courses/${route.params.courseId}`, form.value)
    toast.success('Course updated!')
    router.push(`/admin/courses/${route.params.courseId}`)
  } catch (err) {
...
onMounted(async () => {
  try {
    const [courseRes, categoriesRes] = await Promise.all([
      api.get(`/admin/courses/${route.params.courseId}`),
      api.get('/admin/categories'),
    ])
    const c = courseRes.data
    form.value = { name: c.name, description: c.description, price: c.price, thumbnailUrl: c.thumbnailUrl, difficulty: c.difficulty, status: c.status, CategoryId: c.CategoryId }
