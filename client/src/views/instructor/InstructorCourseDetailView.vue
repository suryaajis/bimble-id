    ...
    <template v-else-if="course">
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          <RouterLink to="/instructor/courses" class="text-gray-400 hover:text-gray-600">
          ...
          <span :class="course.status === 'active' ? 'badge-active' : 'badge-inactive'">{{ course.status === 'active' ? 'published' : 'hidden' }}</span>
        </div>
        <div class="flex gap-2">
          <RouterLink :to="`/instructor/courses/${course.id}/edit`" class="btn-outline text-sm">Edit</RouterLink>
          <RouterLink :to="`/instructor/courses/${course.id}/add-video`" class="btn-brand text-sm">+ Add Video</RouterLink>
        </div>
      </div>

      <div v-if="course.approvalStatus === 'pending'" class="text-sm text-yellow-800 bg-yellow-50 border border-yellow-100 rounded-xl px-4 py-3 mb-6">
        ⏳ This course is awaiting admin review and is not yet visible to learners.
      </div>
      <div v-else-if="course.approvalStatus === 'rejected'" class="text-sm text-red-800 bg-red-50 border border-red-100 rounded-xl px-4 py-3 mb-6">
        ❌ This course was rejected. Edit and resubmit it for review.
      </div>
          ...
          <div class="card p-5">
            ...
            <div v-if="course.Videos?.length" class="space-y-2">
              <div v-for="(video, i) in course.Videos" :key="video.id" class="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
                ...
                <div class="flex gap-2">
                  <RouterLink :to="`/instructor/videos/${video.id}/edit`" class="text-xs text-yellow-600 hover:underline font-medium">Edit</RouterLink>
...
onMounted(async () => {
  try {
    const { data } = await api.get(`/instructor/courses/${route.params.courseId}`)
    course.value = data
  } finally {
    loading.value = false
  }
