  ...
  <div class="p-8">
    <div class="max-w-2xl">
      <div class="flex items-center gap-3 mb-6">
        <RouterLink to="/instructor/courses" class="text-gray-400 hover:text-gray-600 transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </RouterLink>
        <h1 class="font-heading text-2xl font-bold text-gray-900">Create New Course</h1>
      </div>

      <p class="text-sm text-gray-500 bg-yellow-50 border border-yellow-100 rounded-xl px-4 py-3 mb-5">
        ⏳ New courses are reviewed by our team before they appear in the public catalog.
      </p>
      ...
      <div class="card p-6">
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <RouterLink to="/instructor/courses" class="btn-outline flex-1 text-center">Cancel</RouterLink>
          </div>
        </form>
      </div>
    </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (hasFiles) {
      ...
      await api.post('/instructor/courses', formData, { headers: { 'Content-Type': 'multipart/form-data' } })
    } else {
      const Videos = videoSlots.value
        .filter(s => s.type === 'youtube' && s.youtubeUrl)
        .map(s => ({ name: s.name, youtubeUrl: s.youtubeUrl }))

      await api.post('/instructor/courses', { ...form.value, youtubeVideos: JSON.stringify(Videos) })
    }

    toast.success('Course created! Pending review.')
